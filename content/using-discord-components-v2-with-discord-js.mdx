...
In March 2025, Discord released Components v2, a great improvement to the message components system. Before Components v2, a bot could send embeds or text, but it had very limited control over the layout. With the new Components v2, messages are constructed entirely using components. This unified approach eliminates the use of traditional message content and embeds, offering developers greater flexibility in designing message layouts.

Rather than describing everything Components v2 offers, let's learn by example. We'll create a simple bot on Discord using the [Discraft](https://github.com/The-Best-Codes/discraft-js) bot framework to skip the boilerplate code and focus on the components.
...
You should see some logs in the terminal confirming that your bot is online! Fill in your bot Client ID below to get an invite link and invite the bot to a server:
...
> ![Bot Scopes](/image/blog/using-discord-js-cv2/set-bot-scopes.png)
...
Migrating to Components v2 with Discord.js is fairly easy. If you are curious about all the new components, you can see the [list of available components on the Discord.js docs](https://discord.com/developers/docs/components/reference). And don't forget to check out [discord.builders](https://discord.builders/discordjs-javascript-code-generator), which makes it super easy to mockup messages with Components v2.
